...
function getTempsPassageLieu($idEpreuve)
{
    ...
    $query = "SELECT lh.horaire, lh.idInscription, LOWER(lh.lieu) AS lieu, lr.distance_depart, lr.idParcours
...
function getListeEngage($idEpreuve)
{
    global $mysqli;
    $query = "SELECT iei.idInscriptionEpreuveInternaute, iei.idInscriptionEpreuveInternaute AS idInscription,
              iei.dossard, iei.categorie, iei.equipe, iei.idEpreuveParcours,
              ri.nomInternaute, ri.prenomInternaute, ri.sexeInternaute, ri.clubInternaute, ri.villeInternaute,
              ep.nomParcours, ep.horaireDepart
              FROM r_inscriptionepreuveinternaute iei
              JOIN r_internaute ri ON ri.idInternaute = iei.idInternaute
              JOIN r_epreuveparcours ep ON ep.idEpreuveParcours = iei.idEpreuveParcours
              WHERE iei.idEpreuve = ?
              ORDER BY nomInternaute ASC, prenomInternaute ASC";
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    $engagee = mysqli_fetch_all($result, MYSQLI_ASSOC);
    return $engagee;
}

$classement = getListeEngage($idEpreuve);
...
//            ?>
            ...
            <!--            <button id="toggleAbandon" class="btn-abandon" onclick="toggleAbandon()">Afficher uniquement les abandons-->
            <!--            </button>-->
<!--            <button type="button" onclick="togglePredictions()">Afficher/Masquer prédictions</button>-->
            <button type="button" class="btn btn-default"
                    style="background:grey;color:white;font-weight: 500;margin-right:5px;margin-bottom:5px;"
                    onclick="window.location.href='classementLieu.php?idEpreuve=<?php echo $idEpreuve ?>'">Liste des
                résultats
            </button>
            <button type="button" class="btn btn-default"
                    style="background:grey;color:white;font-weight: 500;margin-right:5px;margin-bottom:5px;"
                    onclick="window.location.href='classementListeEngage.php?idEpreuve=<?php echo $idEpreuve ?>'">Liste
                des engagés
            </button>
            <button type="button" class="btn btn-default"
                    style="background:grey;color:white;font-weight: 500;margin-right:5px;margin-bottom:5px;"
                    onclick="window.location.href='classementAbandon.php?idEpreuve=<?php echo $idEpreuve ?>'">Classement
                par abandon
            </button>
...
<!--                                </th>-->
                            ...
                            foreach ($classement as $placeClassement) {
                                ...
                                $nomParcours = $placeClassement['nomParcours'] ?? '';
                                $pparcours = explode("-", $nomParcours);
                                $parcours = $pparcours[0] . (isset($pparcours[1]) ? "<br>" . $pparcours[1] : "");

                                $nbre = 1;
                                $idParcours_test = $placeClassement['idEpreuveParcours'] ?? 0;
                                ...
                                $horaireVal = $placeClassement['horaire'] ?? null;
                                $horaire = $horaireVal ? date('H:i:s', strtotime($horaireVal)) : '-';
                                $temps = $horaireVal ? calculTemps($horaireVal, $placeClassement['horaireDepart'] ?? null) : '-';
                                //on cré les variables ici pour l'affichage des équipes
                                $equipe = $placeClassement['equipe'] ?? 'Aucune';
                                if ($equipe != "Aucune") {
                                    $qequipe = "SELECT nomInternaute, prenomInternaute, sexeInternaute, paysInternaute FROM r_internaute ri
                                                            INNER JOIN r_inscriptionepreuveinternaute iei ON ri.idInternaute = iei.idInternaute
                                                              WHERE iei.equipe LIKE '" . addslashes($equipe) . "' AND iei.idEpreuve = " . $idEpreuve . "";
                                    ...
                                    $nom = "<b>Equipe " . $equipe . "</b></br>" . $nb_femme . " <i class='fa fa-female' style='color:#f50666;'></i> | " . $nb_homme . " <i class='fa fa-male'></i>  (" . $placeClassement['dossard'] . ")";
                                    $club .= "";
                                    $cat .= "";
                                } //on crée les variables ici pour l'affichage des solos
                                else {
                                    $clubInternaute = $placeClassement['clubInternaute'] ?? '';
                                    $villeInternaute = $placeClassement['villeInternaute'] ?? '';
                                    $club = "" . $clubInternaute . "</br>" . $villeInternaute . "";
                                    ...
                                    $idIEI = $placeClassement["idInscriptionEpreuveInternaute"] ?? '';
                                    $nom = "<b>" . $placeClassement['prenomInternaute'] . "</span>&nbsp;<span id='prenom'>" . $placeClassement['nomInternaute'] . "</b></br>" . $cat . "</br>" . $idIEI . "</span>";
                                }

                                $idInscription = $placeClassement['idInscription'] ?? ($placeClassement['idInscriptionEpreuveInternaute'] ?? '');
                                $status = $placeClassement['status'] ?? '';
                                $passage = $placeClassement['passage'] ?? 0;

                                $tempsParLecteur = getTempsPassageParLecteur($placeClassement['idInscriptionEpreuveInternaute'] ?? 0, $idEpreuve);
                                ...
                                //Affichage normal
                                echo "<tr id='" . $idInscription . "' data-parcours='" . $idParcours_test . "' data-sexe='" . $placeClassement['sexeInternaute'] . "' data-status='" . $status . "'>
<td style='vertical-align:middle;color:black;font-size:17px;font-weight:normal;'>" . $nom . "</td>
<td id='club' style='vertical-align:middle;color:black;font-size:15px;font-weight:normal;'>" . $club . "</td>";
