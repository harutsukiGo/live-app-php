...
function getClassementLieu($idEpreuve)
{
  ...
  ep.nomParcours, ep.idEpreuveParcours, ep.horaireDepart,  liv.id , liv.idInscription,liv.status, liv.passage AS passage, liv.status
...
if (isset($_POST['ajout_status']) && !empty($_POST['coureur_status']) && !empty($_POST['idInscription'])) {
    ...
    if ($coureurInfo) {
        ...
        header("Location: classementAbandon.php?idEpreuve=" . $idEpreuve);
        exit();
    }
}
            ...
            <div class="content-title" style="display : flex; flex-direction: column; justify-content : center;">
                ...
                <div class="input-group input-group-lg">
                    <input class="zoneSaisie" style="width: 70%; height:40px;text-align: center;" type="text"
                           placeholder="Ajoutez un coureur" id="maRecherche">
                </div>
            </div>
...
<td style='vertical-align:middle; '><span id='nom' title='" . $placeClassement['passage'] . "° passage' style='color:#348fe2;font-size:38px;font-weight:normal;'>" . $nbre . ". </span></br><span id='nom' title='" . $placeClassement['passage'] . "° passage' style='color:#348fe2;font-size:12px;font-weight:normal;'>" . $placeClassement['passage'] . "° tour </span></td>" .
                                ...
                                if ($status == 'ABD') {
                                    ...
                                    if ($dernierPassage && isset($dernierPassage['horaire'])) {
                                        $heureAbandon = date('H:i:s', strtotime($dernierPassage['horaire']));
                                        $heureInfo = "<br><small>à " . $heureAbandon . "</small>";
                                    }
                                } elseif ($status == 'NP') {
                                    ...
                                    $statutMsg = "En retard";
                                    $colorStatut = "orange";
                                    if ($prochainLieu && isset($prochainLieu['date_max'])) {
                                        $now = new DateTime();
                                        $dateMax = new DateTime($prochainLieu['date_max']);
                                        if ($now > $dateMax) {
                                            $diff = $now->diff($dateMax);
                                            $minutesRetard = ($diff->h * 60) + $diff->i;
                                            $heureInfo = "<br><small>Retard: " . $minutesRetard . " min</small>";
                                        }
                                    }
                                } elseif ($status == 'I') {
                                    $statutMsg = "Inscrit";
                                    ...
                                    if (!$prochainLieu && $dernierPassage) {
                                        $statutMsg = "Terminé";
                                        $colorStatut = "#28a745";
                                        if (isset($dernierPassage['horaire'])) {
                                            $heureArrivee = date('H:i:s', strtotime($dernierPassage['horaire']));
                                            $heureInfo = "<br><small>Arrivée: " . $heureArrivee . "</small>";
                                        }
                                    } elseif ($dernierPassage) {
                                        $statutMsg = "En course";
                                        $colorStatut = "#34c38f";
                                    } else {
        ...
        ";
                                ?>
                                <td> <?php echo $placeClassement['status'] ?></td>

                                <td>
                                    <form method="POST" action="">
                                        <input type="hidden" name="idInscription" value="<?= $placeClassement['idInscription'] ?>">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="coureur_status">Choisir un statut</label>
                                                    <select class="form-control" id="coureur_status" name="coureur_status" required>
                                                        <option value="OK" <?= $placeClassement['status'] == 'OK' ? 'selected' : '' ?>>OK</option>
                                                        <option value="ABD" <?= $placeClassement['status'] == 'ABD' ? 'selected' : '' ?>>Abandon</option>
                                                        <option value="NP" <?= $placeClassement['status'] == 'NP' ? 'selected' : '' ?>>Non partant</option>
                                                        <option value="R" <?= $placeClassement['status'] == 'R' ? 'selected' : '' ?>>En retard</option>
                                                        <option value="I" <?= $placeClassement['status'] == 'I' ? 'selected' : '' ?>>Inscrit</option>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group" style="padding-top: 25px;">
                                                    <button type="submit" name="ajout_status" class="btn btn-warning"
